...
class Model(object):
    def __init__(self, config):
        ...
        # 获取模型中所有的训练参数。(先获取模型中所有的可训练参数，包括bert的可训练参数)
        tvars = tf.trainable_variables()
        # 加载BERT模型（这里的assignment_map是找出bert的可训练参数，因为tf.train.list_variables是获得所有变量，包括参与训练与不参与训练的）
        (assignment_map, initialized_variable_names) = modeling.get_assignment_map_from_checkpoint(tvars,
                                                                                                   init_checkpoint)
        #加载bert中可训练参数，init_checkpoint存放的就是bert模型的预训练参数
